...
static GLvec3s makeVec3s(GLshort x, GLshort y, GLshort z)
{
	GLvec3s v;
	v.x = x;
	v.y = y;
...
struct CombinedVertex4
{
	CombinedVertex cvs[4];
};
 ...
 */
struct imd {
	uint32 len; // base image data size (used to prevent accesses past base_data)
...
struct DrawCommandList {
	...
	scr_coord_val max_y;
	DrawCommandList() {}
	std::vector< CombinedVertex4 > vertices;
	void clear()
	{
		vertices.clear();
	}
	                    ...
	                    GLcolorf alpha, GLcolorf color)
	{
		...
		CombinedVertex4 cv4 = { {
				{
					toVec4( alpha ), toVec4( color ),
					makeVec2f( tx1, ty1 ),
					makeVec2f( ax1, ay1 ),
					makeVec3s( vx1, vy1, vz )
				},
				{
					toVec4( alpha ), toVec4( color ),
					makeVec2f( tx2, ty1 ),
					makeVec2f( ax2, ay1 ),
					makeVec3s( vx2, vy1, vz )
				},
				{
					toVec4( alpha ), toVec4( color ),
					makeVec2f( tx1, ty2 ),
					makeVec2f( ax1, ay2 ),
					makeVec3s( vx1, vy2, vz )
				...
				},
				{
					...
					makeVec2f( tx2, ty2 ),
					makeVec2f( ax2, ay2 ),
					makeVec3s( vx2, vy2, vz )
				}
			}
		};

		vertices.emplace_back( std::move( cv4 ) );
	}
...
static void flushDrawCommands(DrawCommandKey const &key,
                              ...
                              CombinedVertex4  const *vertices,
                              size_t cmd_count)
{
	//when keeping the buffer around, we want GL_DYNAMIC_DRAW.
	glBufferData( GL_ARRAY_BUFFER, cmd_count * sizeof(CombinedVertex4), vertices, GL_DYNAMIC_DRAW );
	...
	for(  unsigned int batchno = 0;
		...
		for(  unsigned int uoid = 0; uoid < MAX_THREADS; uoid++  ) {
			...
			for(  auto &el : batch.unordered_list DCB_UNORDERED_ARRAY_INDEX( uoid )  ) {
				unsigned vno = 0;
				std::size_t vcount = el.second.vertices.size();
				CombinedVertex4 const *data = el.second.vertices.data();
				while(  vno < vcount  ) {
...
static void rezoom_img(const image_id n)
{
	...
	if(  images[n].flags & FLAG_ZOOMABLE  ) {
		if(  images[n].zoom_den == g_simgraphgl.zoom_den[zoom_factor] &&
		                images[n].zoom_num == g_simgraphgl.zoom_num[zoom_factor]  ) {
			return;
		}
		images[n].zoom_den = g_simgraphgl.zoom_den[zoom_factor];
		images[n].zoom_num = g_simgraphgl.zoom_num[zoom_factor];
...
static image_id simgraphgl_register_image(const image_t *image_in)
{
	...
	if(  image_in->len == 0 || image_in->h == 0  ) {
		dbg->warning( "register_image()", "Ignoring image %d because of missing data", images.size() );
		return IMG_EMPTY;
	}

	const image_id id = images.size();
	images.resize( id + 1 );
	image = &images.back();
...
static scr_rect simgraphgl_get_image_offset(image_id image)
{
	if(  image < images.size()  ) {
		...
		return scr_rect{
			(scr_coord_val)floor( images[image].base_x * zoom ),
			(scr_coord_val)floor( images[image].base_y * zoom ),
			(scr_coord_val)ceil( images[image].base_w * zoom ),
			(scr_coord_val)ceil( images[image].base_h * zoom )
		};
	}
	else {
...
static void simgraphgl_draw_rect_colors_clipped(scr_coord_val xp, scr_coord_val yp, scr_coord_val w, scr_coord_val h, PIXVAL *colors, scr_coord_val num_colors, bool horizontal, bool /*dirty*/  CLIP_NUM_DEF)
{
	if (horizontal) {
		...
		for (scr_coord_val i = 0; i < num_colors; i++) {
			...
			display_fb_internal(xp+last_w, yp, next_w-last_w, h, colors[i], CR.clip_rect.x, CR.clip_rect.xx, CR.clip_rect.y, CR.clip_rect.yy);
			last_w = next_w;
		}
	}
	else {
		...
		for (scr_coord_val i = 0; i < num_colors; i++) {
			...
			display_fb_internal(xp, yp+last_h, w, next_h-last_h, colors[i], CR.clip_rect.x, CR.clip_rect.xx, CR.clip_rect.y, CR.clip_rect.yy);
			last_h = next_h;
		}
	}
}
